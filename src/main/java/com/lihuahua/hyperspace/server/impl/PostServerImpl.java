...
public class PostServerImpl extends ServiceImpl<PostMapper, Post> implements PostServer {
    ...
    @Override
    public List<PostComment> getPostComments(String postId, String userId) {
        try {
            // 尝试从Redis缓存中获取评论列表
    ...
    public String uploadCommentImage(MultipartFile file) {
        try {
            ...
            return ossUtil.uploadFileToOSS(file, "comment");
        } catch (Exception e) {
    ...
    @Override
    @Transactional
    public PostComment createComment(String postId, PostComment comment) {
        try {
            // 设置评论ID和帖子ID
    ...
    @Override
    @Transactional
    public PostComment replyComment(String commentId, PostComment comment) {
        try {
            // 获取被回复的评论
            com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<PostComment> queryWrapper =
                    new com.baomidou.mybatisplus.core.conditions.query.QueryWrapper<>();
            ...
            if (parentComment != null) {
                ...
                if (saved) {
                    ...
                    Post post = this.getOne(postQueryWrapper);
                    if (post != null) {
                        post.setCommentCount(post.getCommentCount() + 1);
                        post.setUpdateTime(new Date());
                        this.updateById(post);
