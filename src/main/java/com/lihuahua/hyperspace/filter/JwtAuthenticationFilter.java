...
public class JwtAuthenticationFilter extends OncePerRequestFilter implements ChannelInterceptor {
                                    ...
                                    FilterChain filterChain) throws ServletException, IOException
    {
        ...
        logger.info("收到请求: " + requestURI);
        if (isPublicPath(requestURI)) {
            logger.info("请求路径 " + requestURI + " 是公开路径，跳过JWT验证");
            filterChain.doFilter(request, response);
            return;
        }
        logger.info("请求路径 " + requestURI + " 不是公开路径，进行JWT验证");
            ...
            if (authorization != null && !authorization.isEmpty()) {
                ...
                if (authHeader != null && authHeader.startsWith("Bearer ")) {
                    ...
                    if (userId != null) {
                        ...
                        // Token无效，记录日志但仍允许连接
                        logger.warn("无效的JWT令牌，允许匿名连接");
                    }
                }
            } else {
                // 没有提供Token，允许匿名连接
                logger.debug("未提供JWT令牌，允许匿名连接");
            }
        }
